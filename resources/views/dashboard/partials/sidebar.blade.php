            ...
            <div class="flex-1 overflow-auto {{ $isRtl ? 'pl-1' : 'pr-1' }}">
                ...
                <nav class="space-y-2">
                    ...
                    <details class="group">
                        ...
                        <div class="mt-1 space-y-1 {{ $subListPadding }}">
                            <a href="{{ url('dashboard/reports/cars') }}"
                                class="block py-2 rounded-md text-sm {{ request()->is('dashboard/reports/cars') ? 'text-black font-semibold' : 'text-gray-600 hover:text-black' }}">
                                • {{ __('messages.sidebar.cars_report') }}
                            </a>

                            <a href="{{ url('dashboard/reports/bookings') }}"
                                class="block py-2 rounded-md text-sm {{ request()->is('dashboard/reports/bookings') ? 'text-black font-semibold' : 'text-gray-600 hover:text-black' }}">
                    ...
                    <a href="{{ route('dashboard.admin.employees.index') }}"
                        class="{{ $linkBase }} {{ $linkDirectional }} {{ (request()->routeIs('dashboard.admin.*') && request()->is('dashboard/users*')) ? 'bg-indigo-50 text-black font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="{{ route('employee.bookings.my') }}"
                        class="{{ $linkBase }} {{ $linkDirectional }} {{ request()->is('dashboard/bookings/my') ? 'bg-indigo-50 text-black font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <rect x="3" y="5" width="18" height="16" rx="2" ry="2" />
                            <path d="M16 3v4M8 3v4M3 11h18" />
                        </svg>
                    ...
                    <a href="{{ route('employee.bookings.pending') }}"
                        class="{{ $linkBase }} {{ $linkDirectional }} {{ request()->is('dashboard/bookings/pending') ? 'bg-indigo-50 text-black font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
